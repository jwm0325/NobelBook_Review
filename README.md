...
학습 결과 Loss는 꾸준히 줄고, Accuracy는 늘어나고 있다. Epoch 3에서 검증 정확도 최고점을 찍고 Epoch 4에서는 약간 떨어지는 모습봤을 때  
과적합 징후가 있어 **Epoch 3의 가중치**를 최종 모델로 선정했다
...
### 6-3. 프로젝트 의의
본 프로젝트는 막연하게 느껴졌던 '문학성'이라는 추상적 가치를 **2만여 건의 리뷰 데이터와 딥러닝 모델**을 통해 정량적으로 시각화했다는 데 의의가 있다. 노벨상은 '베스트셀러'를 뽑는 상이 아니라, **'시대를 관통하는 질문'**을 던지는 작가에게 주어진다는 것을 데이터로 증명했다.
